...
namespace EmgenexBusinessPortal.Areas.Business.Controllers
{
    ...
    public class TaskTypesController : BaseController
    {
        ...
        {
            var lookupTaskTypes = db.LookupTaskTypes.Where(a=>a.BusinessId==CurrentUser.BusinessId).Include(l => l.User).Include(l => l.User1);
            return View(lookupTaskTypes.OrderByDescending(a=>a.CreatedOn).ToList());
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,TaskType,CreatedOn,BusinessId,CreatedBy,UpdatedOn,UpdatedBy")] LookupTaskType lookupTaskType)
        {
            if (ModelState.IsValid)
            {
