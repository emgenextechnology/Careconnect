...
{
    [RoutePrefix("{Type:regex(task|tasks)}")]
    [ApiAuthorize]
    public class ApiTaskController : ApiBaseController
    {
        ...
        public IHttpActionResult GetByFilter(FilterTask filter)
        {
            ...
            if (HasRight(allowedRoles) || HasRight(superRoles))
            {
                ...
                if (filter == null)
                    filter = new FilterTask { PageSize = 25, CurrentPage = 1 };
                var response = repository.GetTasks(filter, CurrentUserId, CurrentBusinessId, HasRight(superRoles), HasRight(taskDeletePrivileges));
            ...
            if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        var ReturnUrl = ConfigurationManager.AppSettings["BaseUrl"] + CurrentUser.BusinessName.Replace(" ", "-") + "#/tasks/" + response.Model.ReferenceNumber;
                        ...
                        if (model.PriorityTypeId.HasValue)
                            ...
                            priorityType = EnumHelper.GetEnumName<TaskPriorities>((int)TaskPriorities.Low);
                ...
                response = new RepositoryTask().GetTaskById(TaskId, CurrentUserId, CurrentBusinessId);
        ...
        [Route("{taskid}/setstatus/{statusid}")]
        ...
        private string SaveFile(string base64String, string fileName, int taskId)
        {
            ...
            fileNameWithoutExt = Regex.Replace(fileNameWithoutExt, "[^a-zA-Z0-9.]+", "_", RegexOptions.Compiled);
